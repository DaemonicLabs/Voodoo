...
object Murmur2Hash {
    ...
    fun computeHash(input: String, normalizeWhitespace: Boolean): Long {
        return Murmur2Hash.computeHash(input.toByteArray(StandardCharsets.UTF_8), normalizeWhitespace)
    }

    @Throws(IOException::class)
    fun computeHash(input: ByteArray, normalizeWhitespace: Boolean): Long {
        return Murmur2Hash.computeHash(ByteArrayInputStream(input), 0L, normalizeWhitespace)
    }
    ...
    fun computeNormalizedLength(input: InputStream, buffer: ByteArray?): Long {
        ...
        while (true) {
            ...
            for (index in 0 until bytesRead) {
                if (!Murmur2Hash.isWhitespaceCharacter(buffer[index].toInt())) ++len
            }
        }
    }
