    ...
    override suspend fun pack(
        ...
        val instanceZipVersioned = output.resolve(
            modpack.title?.let { title ->
                "$title ${versionAlias ?: modpack.version} LOCAL.zip"
            } ?: "${modpack.id}-${versionAlias ?: modpack.version} LOCAL.zip"
        )
        val instanceZip = output.resolve("${modpack.title ?: modpack.id} LOCAL.zip")
