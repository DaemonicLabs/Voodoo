...
class CreateVersionCommand : CliktCommand(
    ...
    override fun run(): Unit = withLoggingContext("command" to commandName) {
        runBlocking(MDCContext()) {
            ...
            require(baseDir.exists() && baseDir.isDirectory && metaPackFile.exists()) { "$baseDir must exist and be a directory contains ${metaPackFile.name}" }
            ...
            val versionPacks = baseDir
                ...
                .map { versionPackFilename ->
                    ...
                    VersionPack.parse(versionPackFile)
                }
