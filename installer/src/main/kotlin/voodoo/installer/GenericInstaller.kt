...
/**
 * Created by nikky on 06/084/21.
 * @author Nikky
 */

object GenericInstaller {
    private val logger = KotlinLogging.logger {}
    private val directories = Directories.get(moduleName = "INSTALLER-GENERIC")
    ...
    private suspend fun selfupdate(instanceDir: File, installerUrl: String): File? {
        logger.info { "installer url from modpack: $installerUrl" }
        val voodooFolder = instanceDir.resolve(".voodoo").apply { mkdirs() }
        val currentJarFile = File(GenericInstaller::class.java.protectionDomain.codeSource.location.toURI())
        ...
        newFile.copyTo(voodooFolder.resolve("new.jar"), overwrite = true)
        return newFile
    }

    internal suspend fun install() {
        val instanceDir = File(".")
        val mcDirPointer = File("voodoo.folder.txt")
        if(!mcDirPointer.exists()) error("cannot read $mcDirPointer")
        val minecraftDir = File(".").resolve(mcDirPointer.readText().trim())
        logger.info { "installing into $minecraftDir" }
