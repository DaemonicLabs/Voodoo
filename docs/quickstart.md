...
# optional
# setup alias for the rest of the tutorial
alias voodoo="java -jar $(readlink -f voodoo-0.6.0-SNAPSHOT.jar)"
...
run the `generateSchema` command once to make it generate a `config.json`
...
```bash
voodoo create pack --id magicpack --mcVersion 1.16.3
```
