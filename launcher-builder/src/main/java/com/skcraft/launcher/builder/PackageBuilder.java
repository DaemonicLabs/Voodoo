...
public class PackageBuilder {
    ...
    private static final java.util.logging.Logger log = java.util.logging.Logger.getLogger(PackageBuilder.class.getName());
    ...
    public PackageBuilder(@NonNull ObjectMapper mapper, @NonNull Manifest manifest) throws IOException {
        if (mapper == null) {
            throw new java.lang.NullPointerException("mapper is marked @NonNull but is null");
        }
        if (manifest == null) {
            throw new java.lang.NullPointerException("manifest is marked @NonNull but is null");
        }
        this.properties = LauncherUtils.loadProperties(LauncherUtils.class, "launcher.properties", "com.skcraft.launcher.propertiesFile");
        this.mapper = mapper;
        ...
        try {
            mavenRepos = mapper.readValue(closer.register(LauncherUtils.class.getResourceAsStream("maven_repos.json")), new TypeReference<List<String>>() {
            });
        } finally {
            closer.close();
        }
    ...
    private void processLoader(LinkedHashSet<Library> loaderLibraries, File file, File librariesDir) throws IOException {
        ...
        try {
            ...
            if (profileEntry != null) {
                ...
                log.warning("The file at " + file.getAbsolutePath() + " did not appear to have an install_profile.json file inside -- is it actually an installer for a mod loader?");
            }
        } finally {
            closer.close();
            jarFile.close();
    ...
    public void readVersionManifest(File path) throws IOException, InterruptedException {
        ...
        if (path.exists()) {
            ...
            URL url = url(String.format(properties.getProperty("versionManifestUrl"), manifest.getGameVersion()));
            log.info("Fetching version manifest from " + url + "...");
            manifest.setVersionManifest(HttpRequest.get(url).execute().expectResponseCode(200).returnContent().asJson(VersionManifest.class));
        }
    }
